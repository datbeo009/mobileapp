...
function createTable(db) {
  db.transaction(
    function(tx) {
      tx.executeSql(
        "CREATE TABLE IF NOT EXISTS rentals (id unique primary key, restaurantname text, restauranttype text, time datetime, price float, servicerating  text, cleanliness  text, foodquality text, notes text, reporter text, noteData text)",
        [],
...
function insertRecords(db, data, callback) {
  if (db) {
    db.transaction(
      function(tx) {
        tx.executeSql(
          "INSERT INTO rentals (id, restaurantname, restauranttype, time, price, servicerating, cleanliness, foodquality, notes, reporter) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
          data,
          function(transaction, result) {
...
function updateRecord(db, data, callback) {
  db.transaction(
    function(tx) {
      tx.executeSql(
        "UPDATE rentals SET restaurantname=?, restauranttype=?, time=?, price=?, servicerating=?, cleanliness=?, foodquality=?, notes=?, reporter=? where id=?",
        data,
        function(transaction, result) {
          console.log(result);
...
function updateNoteData(db, data, callback) {
  db.transaction(
    function(tx) {
      tx.executeSql(
        "UPDATE rentals SET noteData=? where id=?",
        data,
        function(transaction, result) {
          console.log(result);
          console.info("Record Updated Successfully!");
  ...
  bindEvents: function() {
      ...
      if (forms[0].checkValidity() === true) {
        ...
        for (let restaurantname in data) {
          if (restaurantname === "notes") {
            html += `<strong>${restaurantname}</strong> : <span style="white-space: pre;"><br />${data[restaurantname]}</span><br />`;
          } else {
            html += `<strong>${restaurantname}</strong> : <span>${data[restaurantname]}</span><br />`;
          }
        }
    ...
    $(".modal.modal-confirm-add-data .confirm-btn").click(function() {
      const data = [
        ...
        state.data.restauranttype,
        state.data.time,
        state.data.price,
        state.data.servicerating,
        state.data.foodquality,
        state.data.cleanliness,
        state.data.notes,
        state.data.reporter
      ];
...
function displayRentals() {
  ...
  state.rentals
    .filter(rental => {
      if (rental.restaurantname.includes(state.inpuSearch)) return true;
      return false;
    })
    .forEach(element => {
      html += `
    <div class="rental-card card mb-3">
      ...
      <div class="card-body" style="text-align: justify;">
          <div class="row">
              <div class="col-6">
                  <strong style="font-size: 14px;">restauranttype</strong>
                  <p class="info-detail info-detail-${
                    element.id
                  }" style="font-size: 12px;">${element.restauranttype}</p>
              </div>
              ...
              <div class="col-6">
                  <strong style="font-size: 14px;">servicerating</strong>
                  <p class="info-detail info-detail-${
                    element.id
                  }" style="font-size: 12px;">${element.servicerating ||
        "== NONE =="}</p>
              </div>
              
              <div class="col-6">
              <strong style="font-size: 14px;">foodquality</strong>
              <p class="info-detail info-detail-${
                element.id
              }" style="font-size: 12px;">${element.foodquality ||
    "== NONE =="}</p>
          </div>

              <div class="col-6">
                  <strong style="font-size: 14px;">cleanliness</strong>
                  <p class="info-detail info-detail-${
                    element.id
                  }" style="font-size: 12px;">${element.cleanliness ||
        "== NONE =="}</p>
              </div>
        ...
        "== NONE =="}</p>
          ...
          <div class="btn-group float-right" role="group" aria-label="Basic example">
              <button data-id="${
                element.id
              }" type="button" class="note-btn btn btn-info">
...
function bindNoteEvent() {
  $(".rental-card button.note-btn").click(function(e) {
    const id = $(this).data("id");

    getRecord(db, id, () => {
      $(".modal.modal-note-data .modal-title").html('Note for ' + state.rental.restaurantname)
      $(".modal.modal-note-data .note-data").val(state.rental.noteData || '')

      $(".modal.modal-note-data").modal("show");

      $(".modal.modal-note-data .confirm-note-btn").click(function() {
        const dataInput = [
          $(".modal.modal-note-data .note-data").val(),
          state.rental.id
        ];

        updateNoteData(db, dataInput, () => {
          $(".modal.modal-note-data .note-data").val('');
          $(".modal.modal-note-data").modal("hide");
        })
      });
    });
  });
...
function bindEditEvent() {
  $(".rental-card button.edit-btn").click(function(e) {
    ...
    getRecord(db, id, () => {
      $(".modal.modal-confirm-edit-data #restaurantname").val(state.rental.restaurantname);
      $(".modal.modal-confirm-edit-data #restauranttype").val(state.rental.restauranttype);
      $(".modal.modal-confirm-edit-data #servicerating").val(
        state.rental.servicerating
      );
      cleanliness
      $(".modal.modal-confirm-edit-data #cleanliness").val(
        state.rental.cleanliness
      );
      
      $(".modal.modal-confirm-edit-data #foodquality").val(
        state.rental.foodquality
      );
