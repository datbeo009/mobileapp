...
function createTable(db) {
  db.transaction(
    function(tx) {
      tx.executeSql(
        "CREATE TABLE IF NOT EXISTS rentals (id unique primary key, property text, bedrooms text, time datetime, price float, furniture text, notes text, reporter text, noteData text)",
        [],
...
function bindNoteEvent() {
  $(".rental-card button.note-btn").click(function(e) {
    ...
    getRecord(db, id, () => {
      $(".modal.modal-note-data .modal-title").html('Note for ' + state.rental.property)
